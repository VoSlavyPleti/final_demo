...
В самом `description` дословно укажи: «делегированный режим», этот путь, точную
область, чтение `/AGENTS.md`, skill и calibration и структуру групп; короткий
заголовок не является заданием. Канонические `analysis.json` и `result.json`
ведёшь только ты. Дождись всех задач, перепроверь и объедини их файлы, затем
выполни общий reverse pass и сформируй заключение.
...
анализа. Конкретная область и, по возможности, уникальный scratch-путь приходят в
...
`{"scope":"<scope>","groups":[...]}`. Если путь не задан, выведи безопасное имя
из назначенного раздела или диапазона. Канонические
